...
**審查日期**：2026-03-12  
**審查範圍**：`data/gameDataCh1.ts`、`app/play/[chapterId]/[sceneId]/page.tsx`（ch1 相關段落）、`data/ch1ReportConfig.ts`、`data/ch1Monologue.ts`、`data/reasoningByChapter.ts`（ch1）
...
- **內心獨白按鈕**：當 **4 位** NPC 敏感對話**全部**完成，且尚未完成內心獨白時，畫面會出現「內心獨白」按鈕。點擊後從三句 KK 內心旁白中**三選一**，選完設 `ch1_monologue_done` 並依選項增加對應洞察維度（procedure／human／evidence）。
...
| 3 | 放映廳：與林瑞堂、阿順各完成一次敏感對話（二選一 + 內心旁白） | 2/4 敏感完成 |
...
| 7 | （可選）點「內心獨白」按鈕，三選一 | 取得額外洞察 |
| 8 | 點劉隊 →「我想向你報告」→ 打開報告編輯器 |  |
| 9 | 證據：選 3 張且涵蓋時間／流程／痕跡三類 | 2 張來自背包，4 張由檢視 flag 解鎖（見道具精簡建議） |
| 10 | 時間線：5 張卡排成正確順序 | T1→T2→T3→T4→T5 |
| 11 | 版本：選一句補充或「先不用」 |  |
| 12 | 態度宣言：完成詞組填空或拖曳後，四選一態度 |  |
| 13 | 點「進入第二章」 | 導向 ch2 intro，第一章破關 |
